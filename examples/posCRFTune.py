...
def main(config, reporter):
    ...
    dropout = 'std'
    lveg = True
    gaussian_dim = config['gaussian_dim']
    log_dir = 'tensorboard/uden-torch4-v4'

    embedding = "random"
    embedding_path = "/home/zhaoyp/zlw/pos/NeuroNLP2/data/glove/glove.6B/glove.6B.100d.txt"
    if embedding == 'random':
        embedd_dict = None
    else:
    ...
    type_alphabet = conllx_data.create_alphabets("data/alphabets/pos_crf/" + language + '/',
    ...
    if dropout == 'std':
        if lveg:
            network = BiRecurrentConvLVeG(embedd_dim, word_alphabet.size(), char_dim, char_alphabet.size(), num_filters,
                                          ...
                                          p_out=p_out, p_rnn=p_rnn, initializer=initializer, gaussian_dim= gaussian_dim)
        else:
    ...
    if use_tb:
        writer = SummaryWriter(log_dir= log_dir + "/" + network_name)
    ...
    for epoch in range(1, num_epochs + 1):
        print('Epoch %d (%s(%s), learning rate=%.4f, decay rate=%.4f (schedule=%d)): ' % (
            epoch, mode, dropout, lr, decay_rate, schedule))
        train_err = 0.
        train_total = 0.
...
if __name__ == '__main__':
    ...
    tune.run_experiments({
        "my_experiment": {
            ...
            "config": {
                "lr": tune.grid_search([0.03, 0.02, 0.01]),
                "momentum": tune.grid_search([0.5, 0.7, 0.9]),
                "mode": tune.grid_search(['RNN', 'LSTM', 'GRU']),
                "batch_size": tune.grid_search([8, 16, 32]),
                "num_layer": tune.grid_search([1, 2]),
            }
        }
    })