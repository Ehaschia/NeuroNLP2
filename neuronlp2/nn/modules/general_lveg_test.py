...
class lveg(nn.Module):
    def __init__(self, word_size, num_labels, bigram=False, spherical=False,
        ...
        if self.bigram:
            ...
            self.trans_nn_mu = nn.Linear(self.input_size,
                                         self.num_labels * self.num_labels * 2 * self.gaussian_dim * 2 * self.gaussian_dim)
            ...
            self.trans_mat_var = Parameter(
                torch.Tensor(self.num_labels, self.num_labels, 2 * self.gaussian_dim, 2 * gaussian_dim))
    ...
    def reset_parameter(self):
        ...
        if self.bigram:
            ...
            var_size = self.trans_mat_var.size()
            # fixme ?
            squeezed_var = self.trans_mat_var.view(-1, var_size[-2], var_size[-1])
            self.trans_mat_var.weight = torch.bmm(squeezed_var.transpose(1, 2), squeezed_var).view(var_size)
    ...
    def forward(self, input, mask=None):
        ...
        if self.bigram:
            ...
            t_var = self.trans_mat_var.view(1, self.num_labels, self.num_labels, 2 * self.gaussian_dim,
                                            2 * self.gaussian_dim)
        ...
        for i in range(length):
            if self.bigram:
                ...
                if i == 0:
                    ...
                    scale_c, mu_c, var_c = gaussian_multi(mu_p, var_p, s_mu[i], s_var[i])
                    scale_p, mu_p, var_p = general_gaussian_multi(t_mu[i].unsqueeze(1), t_var[i].unsqueeze(1),
                                                                  mu_c.unsqueeze(3), var_c.unsqueeze(3), False)
